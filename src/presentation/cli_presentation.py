...
class CliPresentation(AbstractPresentation):
    ...
    def _parse_arguments(self):
        parser = argparse.ArgumentParser("netdev", description="本プログラムはエクセルからコンフィグファイルを作成するためのプログラムです。")
        subparsers = parser.add_subparsers(help='help for subcommand')
        run_parser = subparsers.add_parser('make_config', help="エクセルファイルからコンフィグファイルを作成する際に使用します。")
        run_parser.set_defaults(command="make_config")
        ...
        run_parser.add_argument("-es", "--exception_sheets", required=True, help="パラメーターシートがエクセルの場合、コンフィグ作成時に参照しないエクセルのシートを指定して下さい。")
